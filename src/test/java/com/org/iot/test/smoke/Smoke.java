...
public class Smoke extends TcConstants{
	...
	@BeforeClass(description="Automation Setup initialization")
	...
	public void setupAutomation() throws IOException{
		...
		Datasource ds = new Datasource(csObj.getdbLogin(),csObj.getdbpassword(),csObj.getdbName(),csObj.getdbHost(),csObj.getdbPort(),csObj.getconnectionURL(),true);
	...
	@AfterClass(description="Closing Database Connection and SSH session")
	public void closeAutomation()throws IOException{
		PropertyConfigurator.configure("log4j.properties");
		Config csObj = new Config();
		logger.info("Automation Closing - closing DB and SSH connections");
		Datasource ds = new Datasource(csObj.getdbLogin(),csObj.getdbpassword(),csObj.getdbName(),csObj.getdbHost(),csObj.getdbPort(),csObj.getconnectionURL(),false);	
	...
	@Test(priority=4, description="Validate whether the application is able to create subscription for a group")
	...
	@Test(priority=5, description="Validate whether the application is able to create subscription for AE")
	...
	@Test(priority=6, description="Validate whether the application is able to create subscription for AE's container")
	...
	@Test(priority=7, description="Validate whether the application is able to access the AE's container data")
	...
	public void updateAE() throws IOException, InvalidFormatException, SQLException{
		...
		try{
			...
			if(resp1.statusCode()==SUCCESSOK){
				...
				Datasource ds = new Datasource(aeObj.getdbLogin(),aeObj.getdbpassword(),aeObj.getdbName(),aeObj.getdbHost(),aeObj.getdbPort(),aeObj.getconnectionURL(),true); 
	...
	public void updateCNT() throws IOException, InvalidFormatException, SQLException{
		...
		try{
			...
			if(resp1.statusCode()==SUCCESSOK){
				...
				Datasource ds = new Datasource(cnObj.getdbLogin(),cnObj.getdbpassword(),cnObj.getdbName(),cnObj.getdbHost(),cnObj.getdbPort(),cnObj.getconnectionURL(),true); 
	...
	@Test(priority=10, description="Validate whether the application is able subscribe to its metatdata")
	public void createSubscriptionMetadata() throws IOException, InvalidFormatException, SQLException{
		PropertyConfigurator.configure("log4j.properties");
		logger.info("Starting Subscribe to metatdata  Test Case");
		Config csObj= new Config();
		TestData tdOBJ = new TestData();
		logger.info("Constructing URL for the request");
		String URL = csObj.getBaseUri()+csObj.getPort()+csObj.getBasepath()+"/"+csObj.getapplicationName();
		logger.info("Constructed URL for the request:\t"+URL);
		logger.info("Generating Headers and parameters");
		try{
			Rand ranObj = new Rand();
			ranObj.setRandomSubscription_META();;
			Response resp1=with().log().all().
					header("X-M2M-Origin", csObj.getOrigin()).
					header("Authorization", csObj.getAuthorization()).
					header("X-M2M-RI",csObj.randomRI()).
					header("Content-Type",csObj.getContentype_SUBS()).
					given().
					body(tdOBJ.getsubscreateBody_Meta()).
					when().
					post(URL);
			...
			if(resp1.statusCode()==CREATED){
				...
				logger.info("Creator is the filter selection in the Database:\t"+creator);
				ResultSet res = ds.Query("Select * from\t" +SUBSCRIPTION+" where creator=\t"+"'"+creator+"'");
				String expectedSUB= tdOBJ.getSubscriptionName_meta();
				logger.info("Subscription Name Expected in the DataBase:\t"+expectedSUB);
				while(res.next()){
